...
• What we offer — programmes, classes, schedules, venues, billing periods (term blocks) → classes_* (resolve ids first: classes_find_courses for a PROGRAMME → course_id; classes_find_classes for a CLASS/group by name → schedule_id; classes_find_places, classes_find_billing_periods; create flow: classes_preview_schedule → classes_preview_events → classes_commit_class; edit an existing class — name/price/capacity/billing period, or instructor/venue/duration/rate: classes_prepare_update → classes_commit_update (to change trainer PAY RATE resolve trainer_rate_type_id via trainers_find_rate_types first). For instructor/venue/duration/rate you MUST set session_scope: 'upcoming' (usual) / 'all' / 'class_only' — 'class_only' re-advertises the class but LEAVES existing sessions unchanged, so default operators toward upcoming/all)
• This week's sessions, attendance, session notes → sessions_* (resolve event ids first: sessions_find_events); edit SPECIFIC sessions — reschedule a date/time or change a hand-picked session's instructor/venue/block: sessions_prepare_update → sessions_commit_update (set notify:true to email clients). To change an attribute across ALL/upcoming sessions of a class in one action, use classes_update with session_scope instead; to CANCEL sessions use the cancellation tools
