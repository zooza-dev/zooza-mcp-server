...
└── tools/             → atomic operations Claude calls automatically (sessions_find_events, cancel_event…)
```
...
| Tool | `classes_find_courses` | Search programmes by name/type |
| Tool | `classes_find_places` | Search venues + rooms |
| Tool | `trainers_find` | Search instructors |
| Tool | `classes_find_billing_periods` | List billing periods |
| Tool | `classes_preview_schedule` | Preview a class plan before creating it |
| Tool | `classes_preview_events` | Calculate session dates before committing |
| Tool | `classes_commit_class` | Create a class (3 API calls → 1 MCP tool) |
| Free tool | `explain_data_model` | Explain Zooza entities (no API) |
| Free tool | `comms_list_merge_vars` | Catalogue of merge variables (no API) |
| Free tool | `classes_list_schedule_patterns` | Recurrence patterns reference (no API) |
...
|------|-------------|-------|
| `sessions_find_events` | `GET /events?date=&trainer_id=&status=scheduled` | Core — returns sessions for a day |
...
### Priority 2 — Skill: `sessions_mark_attendance`
...
| Tool | API endpoint | Notes |
|------|-------------|-------|
| `sessions_find_events` | `GET /events?date=&schedule_id=` | **Shared with cancel_day** — one tool, two uses |
...
|---------|-------------|---------------|--------|
| `/zooza:today` | Today's sessions, instructor coverage, enrolment counts | `sessions_find_events` (new) | Waits for `sessions_find_events` |
...
| `/zooza:attendance` | Open register for the next session | `sessions_find_events`, `get_session_register` | Waits for tools |
...
|------|---------|---------|
| `sessions_find_events` | cancel_day + sessions_mark_attendance + today command | **P0** |
| `issue_makeup_credit` | cancel_day + sessions_mark_attendance | **P0** |
| `update_attendance` | sessions_mark_attendance | **P1** |
| `get_session_register` | sessions_mark_attendance + today command | **P1** |
...
|---------------|---------|---------|
| Read-only | `none` | `sessions_find_events`, `get_session_register` |
| Preview with no change | `preview-only` | `classes_preview_schedule`, `explain_data_model` |
...
|------|--------------------|--------------------|
| "Show today's schedule" | Just display logic composed from sessions_find_events | Command `/zooza:today` |
...
1. **Verb-first naming is the standard** — Linear, Stripe: `create_issue`, `cancel_subscription`. Our `create_class`, `cancel_event`, `sessions_mark_attendance` are correct. *Never* `class_create`.
...
4. **Free tools are rare** — almost no one ships tools without an API call. Our `explain_data_model`, `list_merge_vars`, `classes_list_schedule_patterns` are unique. They reduce context bloat and prevent Claude from hallucinating domain knowledge.

5. **Compound operations > one-to-one API wrapping** — Stripe has `create_payment_intent` (not `create_intent` + `attach_method` + `confirm` as separate tools). Our `classes_commit_class` (3 API calls → 1 MCP tool) follows the same pattern.
...
Step 1 — P0 tools (2–3 days)
  ├── sessions_find_events              (blocks everything else)
  └── issue_makeup_credit      (blocks cancel_day + sessions_mark_attendance)
...
Step 3 — sessions_mark_attendance skill + supporting tools (3–4 days)
...
Step 4 — daily dashboard commands (1–2 days)
  ...
  └── commands/today.md        (needs sessions_find_events — ready after Step 1)
...
**New skills: 3 (cancel_day, sessions_mark_attendance, initiate_refund)**
...
|----------|-------|--------|
| `sessions_find_events` | Tool | Calls API, atomic operation |
...
| `sessions_mark_attendance` | **Skill** | Multi-step, role-based, trialist follow-up logic |
...
| `/zooza:attendance` | **Command** | Triggers sessions_mark_attendance skill |
